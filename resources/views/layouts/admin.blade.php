    ...
    <div id="app">
        ...
        <div class="relative md:ml-64 bg-blueGray-50 min-h-screen">
            ...
            <div class="relative bg-indigo-500 md:pt-32 pb-32 pt-12">
                {{-- This is the blue back ground--}}
                <div class="px-4 md:px-10 mx-auto w-full">&nbsp</div>
            </div>

            <div class="relative px-4 md:px-10 mx-auto w-full min-h-full -m-48">
                @if(session('status'))
                    <x-alert message="{{ session('message') }}" type="{{ session('status') }}" variant="indigo" role="alert" />
                @endif
                @yield('content')
